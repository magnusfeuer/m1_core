...
//  struct DDSHeader {
//    unsigned int mVersion;    // File version
//    unsigned int mImageCount; // Number of images in this cache.
//    unsigned int mHeight;     // Height of images. All images must have same height.
//    unsigned int mWidth;      // Width of images. All images must have same width.
//    unsigned int mPixelType;  // Target pixel type
//  } Header;
//  int imageOffset[mImageCount]; // An offset table containing the start of each image. 
//                                // First image always is offseted at 0. Thus
//                                // imageOffset[0] == 0.
//  
//  After the DDSHeader, there are mImageCount images of varying sizes.
...
//  If a line in an image is blank, a EPIC_ANIM_SKIP block is given 
...
int dds_compress_row(EPixel_t* aRow, DDSImage* aImage, int frame, int y,
		     int aWidth,
		     u_int8_t* target,
		     u_int32_t offset);

int dds_block(EPixel_t* aRow, int x, u_int8_t* target, EAnimPixels* hdr);
...
int analyze_pixel_data(EPixel_t* aRow, int aX, int aWidth, EAnimPixels* header);
...
typedef struct _frame_block
{
    ...
    EAnimPixels* hdr;          /* Either ALPHA (line mode) or COPY/SHADOW */
} FrameBlock;

dds_lhash_t* hblock = NULL;


void dump_block_header(EAnimPixels* b)
{
    u_int8_t* ptr = ((u_int8_t*)b)+sizeof(EAnimPixels);
    int count = b->count;
    int i = 1;
    switch(b->type) {
    case EANIM_RGBA:
...
u_int32_t size_block_header(EAnimPixels* b)
{
    int sz = 0;
...
int cmp_block_header(EAnimPixels* b1, EAnimPixels* b2)
{
    u_int8_t* b1_ptr;
    u_int8_t* b2_ptr;
    ...
    b1_ptr = ((u_int8_t*) b1) + sizeof(EAnimPixels);
    b2_ptr = ((u_int8_t*) b2) + sizeof(EAnimPixels);
    switch(b1->type) {
    case EANIM_RGBA:
...
EAnimPixels* copy_block_header(EAnimPixels* b)
{
    EAnimPixels* b_copy = NULL;

    switch(b->type) {
    case EANIM_BGRA:
    case EANIM_RGBA:
	b_copy = (EAnimPixels*) malloc(sizeof(EAnimPixels) + 4*b->count);
	memcpy(b_copy, b, sizeof(EAnimPixels) + 4*b->count);

    case EANIM_COPY:
	b_copy = (EAnimPixels*) malloc(sizeof(EAnimPixels) + 
				       bytesPerPixel*b->count);
	memcpy(b_copy, b, sizeof(EAnimPixels) + bytesPerPixel*b->count);
	break;
    case EANIM_FILL:
	b_copy = (EAnimPixels*) malloc(sizeof(EAnimPixels) + 4);
	memcpy(b_copy, b, sizeof(EAnimPixels) + 4);
	break;
    case EANIM_SHADOW:
	b_copy = (EAnimPixels*) malloc(sizeof(EAnimPixels) + b->count);
	memcpy(b_copy, b, sizeof(EAnimPixels) + b->count);
	break;
    case EANIM_SKIP:
	b_copy = (EAnimPixels*) malloc(sizeof(EAnimPixels));
	memcpy(b_copy, b, sizeof(EAnimPixels));	
    }
    return b_copy;
}
...
hash_value_t hash_block_header(EAnimPixels* b)
{
    hash_value_t h = 0;
    u_int8_t* ptr;
    ...
    /* then select method */
    ptr = ((u_int8_t*) b)+sizeof(EAnimPixels);
    switch(b->type) {
    case EANIM_BGRA:
    case EANIM_RGBA:
 ...
 */
FrameBlock* insert_block(int frame, int y, int offset, EAnimPixels* hdr)
{
    FrameBlock bt;
    FrameBlock* b;
    /* create template */
...
FrameBlock* find_block(int frame, EAnimPixels* hdr)
{
    FrameBlock bt;
    FrameBlock* b;    
...
EPixmap* scale_pixmap(EPixmap* aSrcImage, int aWidth, int aHeight, int aAlpha)
{
    EPixmap*  dstImage;
    ...
    dstImage = EPixmapCreate(aWidth, aHeight, aSrcImage->pixelType);
    if (!dstImage)
	return NULL;
    ...
    dstLineSize[0] = dstImage->bytesPerRow;
    ...
    if (aAlpha) {
	EPixmap*  srcAlphaImage = NULL;
	EPixmap*  dstAlphaImage = NULL;
	...
	if ((dstAlphaImage = EPixmapCreate(dstImage->width,
					   dstImage->height,
					   EPIXEL_TYPE_ALPHA)) == 0) {
	    sws_freeContext(ctx2);
	...
	EPixmapCopyArea(dstAlphaImage, dstImage,
			0, 0, 0, 0,
			dstAlphaImage->width,
...
int scale_img(DDSImage* aImg, int aWidth, int aHeight)
{
	...
	EPixmap* pixmap   = aImg->getPixmap(i);
...
int dds_verify(int frame, u_int8_t *target, int size)
{
    ...
    while(size > 0) {
	EAnimPixels* hdr = (EAnimPixels*) target;

	count++;
	target += sizeof(EAnimPixels);
	size -= sizeof(EAnimPixels);
	
	switch(hdr->type) {
	case EANIM_SKIP:
	    if (hdr->count == 0) {
		printf("EANIM_SKIP: Count=0\n");
		return 0;
	    }
	    break;
	case EANIM_SHADOW:
		     ...
		     u_int32_t offset)
{
	...
	hdr = (EAnimPixels *) target;
	hdr->type   = alphaBlockType;
	hdr->count  = aWidth;

	/* build alpha block */
	bsize = dds_block(aRow, x, target+sizeof(EAnimPixels), hdr);

	if ((b=find_block(frame, hdr)) != NULL) {
	    hdr->type   = EANIM_INDIRECT;
	    hdr->itype  = b->type;
	    ...
	    ind_byte_count += sizeof(EAnimPixels);
	    DBO("I%c[%d] ", block_id[b->type], b->npixels);
	    return sizeof(EAnimPixels);
	}
	...
	if (indirect && (hdr->count > INDIRECT_THRESHOLD) &&
		...
		break;
	    case EANIM_SKIP:
		fprintf(stderr, "EANIM_SKIP: found in cache\n");
		exit(1);
	    }
	    /* update target and offset */
	    target = save_target + sizeof(EAnimPixels);
	    offset = save_offset + sizeof(EAnimPixels);
	}
	else {
	    if (!indirect && (b != NULL)) {
		if (b->type == 0)
	    ...
	    if (indirect && (hdr->type != EANIM_SKIP))
		...
		shadow_byte_count += (sizeof(EAnimPixels) + 
				      hdr->count);
		break;
	    case EANIM_FILL:
		fill_block_count++;
		fill_pixels += 1;
		fill_byte_count += (4 + sizeof(EAnimPixels));
		break;
	    case EANIM_SKIP:
		skip_block_count++;
		skip_pixels += hdr->count;
		skip_byte_count += sizeof(EAnimPixels);
		break;
	    }
	}
    }
...
//
int count_transparent_pixels(EPixel_t* aRow,int aX,int aWidth)
{
    int count = 0;
...
//
int count_equal_pixels(EPixel_t* aRow, EPixel_t p0, int aX, int aWidth)
{
    int count = 0;
    while(aX < aWidth) {
	EPixel_t p1 = aRow[aX];
...
int count_fill_pixels(EPixel_t* aRow,int aX,int aWidth)
{
    if (aX < aWidth)
...
//
int count_opaque_pixels(EPixel_t* aRow,int aX,int aWidth)
{
    int count = 0;
...
//
int count_shadow_pixels(EPixel_t* aRow, int aX, int aWidth)
{
    int count = 0;
...
int main(int argc, char *argv[])
{
    ...
    int tot_block_count;
    int verify = false;
    char* alpha_block_type = "????";
    
    pixelType = -1;

    printf("DDS compiler version %s. (C) 2006 Magden LLC\n", VERSION);
    //
    // Do the option thing
    //
	...
	case 'C':
		...
		pixelType = EPIXEL_TYPE_RGBA;
		alphaBlockType = EANIM_RGBA;
	    }
	    else if (strcmp(optarg, "argb") == 0) {
		pixelType = EPIXEL_TYPE_ARGB;
		alphaBlockType = EANIM_RGBA;
	    }
	    else if (strcmp(optarg, "rgb") == 0) {
		pixelType = EPIXEL_TYPE_RGB;
		alphaBlockType = EANIM_RGBA;
	    }
	    else if (strcmp(optarg, "bgra") == 0) {
		pixelType = EPIXEL_TYPE_BGRA;
		alphaBlockType = EANIM_BGRA;
	    }
	    else if (strcmp(optarg, "abgr") == 0) {
		pixelType = EPIXEL_TYPE_ABGR;
		alphaBlockType = EANIM_BGRA;
	    }
	    else if (strcmp(optarg, "bgr") == 0) {
		pixelType = EPIXEL_TYPE_BGR;
		alphaBlockType = EANIM_BGRA;
	    }
	    break;
	case 'S':
    ...
    bytesPerPixel = EPIXEL_SIZE(pixelType);
    
    if (use_indirect) {
	hblock = dds_lhash_new("idirect", 2, hash_block, cmp_block,
    ...
    //
    cache_res.mVersion = DDS_FORMAT_VERSION;
    cache_res.mImageCount = 0;
    cache_res.mHeight = 0;
    cache_res.mWidth = 0;
    cache_res.mPixelType = 0;
    ...
    write(out, (char *) &cache_res, sizeof(cache_res));
  
    //
    // Write out a blank offset table.
    //
    dummy = (char *) malloc(sizeof(int) * (image_count/stepping));
    write(out, dummy, sizeof(int) * (image_count/stepping));
	...
	for (frame = 0; frame < img->frames(); frame++) {
	    ...
	    // 
	    lseek(out, sizeof(CDDSHeader) + (sizeof(int)*ind), SEEK_SET);
	    write(out, (char *) &offset, sizeof(int));
	    lseek(out, 0, SEEK_END);
	    //
	    // Dump the image data.
	    //
	    write(out, image_data, image_size);
	    if (debug_output)
    ...
    cache_res.mVersion = DDS_FORMAT_VERSION; // Version of the format used in this DDS file.
    cache_res.mHeight = height;
    cache_res.mWidth = width;
    cache_res.mImageCount = image_count;
    cache_res.mPixelType = pixelType;
    write(out, (char *) &cache_res, sizeof(cache_res));
    close(out);
    ...
    if (alphaBlockType==EANIM_RGBA)
	alpha_block_type = "RGBA";
    else if (alphaBlockType==EANIM_BGRA)
