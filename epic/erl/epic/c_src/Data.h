...
static inline void data_reset(Data* data, u_int32_t skip)
{
...
static inline Data* data_new(u_int8_t* buf, u_int32_t len, u_int32_t skip)
{
...
static inline void data_free(Data* data)
{
    data_final(data);
    free(data);
}
...
static inline void data_send(Data* data, int fd)
{
...
static inline void put_string(Data* data, const char* string, int n)
{
    if ((string == NULL) || (n == 0)) {
	u_int8_t* ptr = data_alloc(data, 2);
	*ptr++ = STRING1;
...
static inline int get_float32(Data* data, float* val)
{
    union { float f32; u_int32_t u32; } fu;
    if (data_avail(data) < sizeof(float)) return 0;
    fu.u32 = GET_UINT32(data->ptr);
    *val = fu.f32;
    data->ptr += sizeof(float);
    return 1;
}

static inline int get_float64(Data* data, double* val)
{
    union { double f64; u_int32_t u32[2]; } fu;
    if (data_avail(data) < sizeof(double)) return 0;
    fu.u32[_QUAD_HIGHWORD] = GET_UINT32(data->ptr);
    fu.u32[_QUAD_LOWWORD]  = GET_UINT32(data->ptr+4);
    *val = fu.f64;
    data->ptr += sizeof(double);
    return 1;
}
