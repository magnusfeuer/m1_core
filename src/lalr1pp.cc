...
[    typedef int semantic_type;])[
    ...
    /// Parse token.
    /// \returns  0 iff parsing succeeded.
    int parse_token (int yychar, semantic_type yylval, location yylloc);

    virtual void accept(void);
    virtual void abort(void);
    virtual void error(void);

    ]b4_parser_class_name[::semantic_type top(void);
  ...
  private:
    int parse_token0 (int yychar, semantic_type yylval, location yylloc);
...
[       default:
  ...
  void
  ]b4_parser_class_name[::accept (void)
  {
  }

  void
  ]b4_parser_class_name[::abort (void)
  {
  }

  void
  ]b4_parser_class_name[::error (void)
  {
  }

  ]b4_parser_class_name[::semantic_type
  ]b4_parser_class_name[::top (void)
  {
      return yysemantic_stack_[0];
  }
...
[  
      ...
      /* initialize state - FIXME check return value */
      res = parse_token0(yyempty_, yylval, yylloc);
      need_init_ = false;
...
m4_ifdef([b4_lex_param], [, ]b4_lex_param))[;
	  res = parse_token0(yychar, yylval, yylloc);
      }
      return res;
  }

  int
  ]b4_parser_class_name[::parse_token (int yychar, semantic_type yylval, location yylloc)
  {
    if (need_init_) {
      int init_res;
      YYCDEBUG << "Starting parse" << std::endl;
      ...
      [
       ...
       yylocation_stack_.push (yylloc);
       /* initialize state - FIXME check return value */
       init_res = parse_token0(yyempty_, yylval, yylloc);
       need_init_ = false;
       if (init_res != 3)
	   return init_res;
    }
    return parse_token0(yychar, yylval, yylloc);
  }

  int
  ]b4_parser_class_name[::parse_token0 (int yychar, semantic_type yylval, location yylloc)
  {
    int yyresult;
  ...
  yypushtok:
    if (yychar == yyempty_)
	return 3;      /* Read a look-ahead token.  */
